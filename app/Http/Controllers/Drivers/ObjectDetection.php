...
class ObjectDetection extends Controller
{
    /**
     ...
     */
    public static function Identify(string $label, string $path)
    {
        return static::Quantity($path)->where('label', $label);
    }
    ...
    public static function Apply(string $file)
    {
        ...
        foreach ($itens->toArray() as $item) {
            ...
            $image->drawRectangle($box['xmin'], $box['ymin'], function (RectangleFactory $rectangle) {
        ...
        $image->text("Arthemys - Object Detection Driver", 120, max(10, 0), function (FontFactory $font) {
            $font->filename(public_path("fonts/orbitron.ttf"));
            $font->size(12);
            $font->color('fff');
            $font->stroke('ff5500', 2);
